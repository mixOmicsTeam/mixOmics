    ...
    for (h in 1:ncomp)
    {
        ...
        for (i in 1:M)
        {
            ...
            if (h == 1)
            {
                ...
                spls.res = mixOmics2::spls(X.train[,nzv], Y.train, ncomp = ncomp, mode = mode, max.iter = max.iter, tol = tol, keepX = keepX.temp, keepY = keepY, near.zero.var = FALSE, scale = scale)
