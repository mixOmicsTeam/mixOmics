...
#' @export
#' @rdname block.plsda
setMethod('block.plsda', 'ANY', function(data=NULL, X=NULL, Y=NULL, formula=NULL, ...) {
    ...
    .check_sig_ANY(mc, fun = "block.plsda")
...
#' @export
#' @rdname block.plsda
setMethod('block.plsda', signature(data = 'MultiAssayExperiment'), 
          function(data=NULL, X=NULL, Y=NULL, formula=NULL, ...) {
              ...
              mc[[1L]] <- quote(.block.plsda)
              result <- eval(mc)
              .call_return(result, mc$ret.call, mcr = match.call(), fun.name = 'block.plsda')
          })
...
#' @export
#' @rdname block.plsda
setMethod('block.plsda', signature(data = 'MatchedAssayExperiment'), 
          function(data=NULL, X=NULL, Y=NULL, formula=NULL, ...) {
              tryCatch(mget(names(formals()), sys.frame(sys.nframe())),
                       error = function(e) stop(e$message, call. = FALSE))
              mc <- match.call()
...
#' @export
#' @rdname block.plsda
setMethod('block.plsda', signature(formula = 'formula'), 
          function(data=NULL, X=NULL, Y=NULL, formula=NULL, ...) {
              tryCatch(mget(names(formals()), sys.frame(sys.nframe())),
                       error = function(e) stop(e$message, call. = FALSE))
              mc <- match.call()
