...
* bug fix implemented for [#399](https://github.com/mixOmics-org/mixOmics/issues/399) `rcc()` and `tune.rcc()` gain a `scale` argument (default `TRUE`). Ridge rCCA is not invariant to variable scaling, so `X` and `Y` are now standardised before regularisation, and `tune.rcc()` scales held-out samples with the training-fold statistics. Default ridge results change; use `scale = FALSE` to reproduce previous results
* bug fix implemented for [#398](https://github.com/mixOmics-org/mixOmics/issues/398) `network()` and `cimDiablo()` documentation now states the actual default of `comp` (all components)
* GitHub repository moved to the `mixOmics-org` organisation; README links and badges updated ([PR #401](https://github.com/mixOmics-org/mixOmics/pull/401))

#### August 2026

* bug fix implemented in [PR #386](https://github.com/mixOmics-org/mixOmics/pull/386) `mint.block.(s)pls()` and `mint.block.(s)plsda()` now return correctly structured objects (`indY`, `design`, `weights`, `keepX`/`keepY`), so `predict()` works on them; `keepX`/`keepY` also fixed in `block.spls()`

#### May 2026

* bug fix implemented for [#377](https://github.com/mixOmicsTeam/mixOmics/issues/377) fixed vector `xlim` handling in the `graphics` branch of `plotLoadings()` contribution plots
* enhancement request [#381](https://github.com/mixOmicsTeam/mixOmics/issues/381) expanded test coverage for model utilities and internal helper paths, with small fixes made for issues exposed by the new tests

#### April 2026
** Version 6.36.0 **

[Bioconductor release version 6.36.0](https://www.bioconductor.org/packages/release/bioc/html/mixOmics.html) released 29th April 2026
mixOmics can now be downloaded using [Bioconductor version 3.23](https://bioconductor.org/news/bioc_3_23_release/) and is compatible with R 4.6.0
...
mixOmics can now be downloaded using [Bioconductor version 3.21](https://bioconductor.org/news/bioc_3_21_release/) and is compatible with R 4.5.0
