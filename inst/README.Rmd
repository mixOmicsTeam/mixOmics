...
`r badge_travis("mixOmicsTeam/mixOmics")`
`r badge_bioc_release('mixOmics', 'green')`
`r badge_devel('mixOmicsTeam/mixOmics', 'orange')`
`r badge_codecov("mixOmicsTeam/mixOmics")`
`r badge_bioc_download("mixOmics", "month", "green")`
...
Check after installation that the following code does not throw any error (especially Mac users - refer to [installation instructions](#installation)) and that the welcome message confirms you have installed `r badge_bioc_release('mixOmics', 'green')`:
...
You can also install the development version (`r badge_devel('mixOmicsTeam/mixOmics', 'orange')`) for new features yet to be widely tested:
```{r}
