...
pkg_version_badge <- sprintf("https://img.shields.io/badge/Package%%20version-%s-orange.svg?style=flat-square", pkg_version)
...
[![packageVersion](`r pkg_version_badge`)](https://github.com/mixOmicsTeam/mixOmics/blob/master/DESCRIPTION)
...
Check after installation that the following code does not throw any error (especially Mac users - refer to [installation instructions](#installation)) and that the welcome message confirms you have installed ![](`r pkg_version_badge`) :
...
You can also install the development version for new features yet to be tested:
```{r}
